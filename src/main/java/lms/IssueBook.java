 ...
 * @author Vlad
 * 
 * Here, the user can issue a book to a student.
 */
public class IssueBook extends JFrame implements ActionListener {
	...
	/**
	 * Main function for issue book page
	 * @param args
	 */
	...
	public IssueBook() {
		...
		JLabel l3 = new JLabel("Publish D");
		...
		panel.setBorder(new TitledBorder(new LineBorder(new Color(55, 145, 225), 2, true), "Issue-Book",
		...
		JLabel l12 = new JLabel("University");
		...
		panel_1.setBorder(new TitledBorder(new LineBorder(new Color(55, 145, 225), 2, true), "Student-Deatails",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 145, 225)));
		...
		try {
			...
			if (ae.getSource() == b1) {
				...
				while (rs.next()) {
					...
					t3.setText(rs.getString("publish_date"));
			...
			if (ae.getSource() == b2) {
				...
				while (rs.next()) {
					...
					t12.setText(rs.getString("college"));
					t11.setText(rs.getString("university"));
			...
			if (ae.getSource() == b3) {
				try {
					String sql = "insert into issueBook(book_id, student_id, bname, sname, college, university, dateOfIssue) values(?, ?, ?, ?, ?, ?, ?)";
					PreparedStatement st = con.c.prepareStatement(sql);
					st.setString(1, t1.getText());
