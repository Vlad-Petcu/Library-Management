...
public class ReturnBook extends JFrame implements ActionListener {
	...
	public ReturnBook() {
		...
		panel.setBorder(new TitledBorder(new LineBorder(new Color(55, 145, 225), 2, true), "Return-Panel",
		...
		try {
			...
			if (ae.getSource() == b1) {
				...
				while (rs.next()) {
					...
					textField_4.setText(rs.getString("college"));
					textField_5.setText(rs.getString("university"));
			...
			if (ae.getSource() == b2) {
				String sql = "insert into returnBook(book_id, student_id, bname, sname, college, university, dateOfIssue, dateOfReturn) values(?, ?, ?, ?, ?, ?, ?, ?)";
				PreparedStatement st = con.c.prepareStatement(sql);
				st.setString(1, textField.getText());
